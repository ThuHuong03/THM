...
export default function AddCustomer({ OnClickCancel }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
        await newCustomer.add(host, getCustomers); // Ensure host is passed here
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
      <div className="bg-white text-black p-8 rounded-lg shadow-lg max-h-[80vh] max-w-[600px] overflow-y-auto">
        <h2 className="text-3xl font-bold mb-4 text-dark-rust">Thêm Khách Hàng</h2>
        ...
        <InputCmp title="Tổng cà tươi" name="totalRawCF" value={formData.totalRawCF} onChange={handleInputChange} placeholder="kg"/>
        <InputCmp title="Tổng cà nhân" name="totalCF" value={formData.totalCF} onChange={handleInputChange} placeholder="kg"/>
        <InputCmp title="Tổng tiêu" name="totalPepper" value={formData.totalPepper} onChange={handleInputChange} placeholder="kg"/>
