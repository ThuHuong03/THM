...
export default function PepperReport() {
  ...
  useEffect(() => {
    async function fetchInvoices() {
      const filteredInvoices = invoices.filter(invoice => invoice.type === "tiêu");
  ...
  const groupByDate = (invoices, selectedDate) => {
    return invoices.reduce((acc, invoice) => {
      ...
      if (!selectedDate || invoiceDate === selectedDate) {
        ...
        acc[invoiceDate][invoice.title === 'Phiếu Xuất' ? 'xuat' : invoice.title=== "Phiếu Nhập"? 'nhap': 'ban'] += invoice.totalAmount;
      }
  ...
  const groupByMonth = (invoices, year) => {
    return invoices.reduce((acc, invoice) => {
      ...
      if (invoiceYear === year) {
        ...
        acc[month][invoice.title === 'Phiếu Xuất' ? 'xuat' : invoice.title=== "Phiếu Nhập"? 'nhap': 'ban'] += invoice.totalAmount;
      }
